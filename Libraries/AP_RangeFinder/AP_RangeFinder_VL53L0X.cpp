...
static bool transfer(uint8_t* send, uint32_t send_len, uint8_t* recv, uint32_t recv_len)
{
  ...
  msgs[0].addr  = VL53L0X_ADDR;
  msgs[0].flags = RT_I2C_WR;
  msgs[0].buf   = send;
  msgs[0].len   = send_len;
  
  msgs[1].addr  = VL53L0X_ADDR;
...
static bool read_registers(uint8_t first_reg, uint8_t* recv, uint32_t recv_len)
{
  struct rt_i2c_msg msgs[2];
...
AP_RangeFinder_Backend *AP_RangeFinder_VL53L0X::detect(RangeFinder::RangeFinder_State &_state)
{
    AP_RangeFinder_VL53L0X *sensor
        = new AP_RangeFinder_VL53L0X(_state);
...
{
    uint8_t v1, v2;
    if (!read_registers(0xC0, &v1, 1) ||
        !read_registers(0xC1, &v2, 1) ||
        v1 != 0xEE ||
        v2 != 0xAA) {
        return false;
    }
...
bool AP_RangeFinder_VL53L0X::init()
{
    ...
    if (!read_registers(GLOBAL_CONFIG_SPAD_ENABLES_REF_0, ref_spad_map, 6)) {
    ...
    transfer(reg_spad_map, 7, nullptr, 0);
