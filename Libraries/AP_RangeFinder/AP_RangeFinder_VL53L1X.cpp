...
/*
   ...
   there.
*/
AP_RangeFinder_Backend *AP_RangeFinder_VL53L1X::detect(RangeFinder::RangeFinder_State &_state, DistanceMode mode)
{
    AP_RangeFinder_VL53L1X *sensor
        = new AP_RangeFinder_VL53L1X(_state);
...
void AP_RangeFinder_VL53L1X::update(void)
{
    ...
    if (counter > 0) {
        ...
        sum_mm = 0;
        counter = 0;
    } else if (HAL_GetTick() - state.last_reading_ms > 200) {
